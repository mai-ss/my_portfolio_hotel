...
$(document).ready(function () {
    $(".header__mainvisual--img-select img").hover(
        function () {
            // 対応する画像を表示
            const imgId = $(this).data("img");
            $(".header__mainvisual--img-room img").stop(true,true).hide(); // 一旦全て非表示
            $(`.header__mainvisual--img-room img[data-id="${imgId}"]`).stop(true,true).fadeIn(500); // 対応する画像を表示
        },
        function () {
            // hoverを外したらデフォルト画像に戻す
            const defaultImage = $(".header__mainvisual--img-room").data("default-img"); // デフォルト画像を取得
            $(".header__mainvisual--img-room img").stop(true,true).hide(); // 一旦全て非表示
            $(`.header__mainvisual--img-room img[data-id="${defaultImage}"]`).stop(true,true).fadeIn(500); // デフォルト画像を表示
        }
    );
});